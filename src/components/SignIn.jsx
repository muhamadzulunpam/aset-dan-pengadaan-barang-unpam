...
const SignIn = () => {
  ...
  const demoAccounts = [
    { email: 'admin@procurement.com', password: 'password', role: 'Admin' },
    { email: 'user@procurement.com', password: 'password', role: 'User' }
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-cyan-50/30 flex items-center justify-center p-4">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="bg-blue-50/50 border border-blue-200 rounded-xl p-4">
          <h3 className="text-sm font-semibold text-blue-900 mb-2">Akun Demo:</h3>
          <div className="space-y-2">
            {demoAccounts.map((account, index) => (
              <button
                key={index}
                onClick={() => fillDemoAccount(account.email, account.password)}
                className="w-full text-left p-2 text-sm text-blue-700 hover:bg-blue-100 rounded-lg transition-colors"
              >
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6 bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/60 p-8 shadow-sm">
          ...
          <div className="text-center">
            <p className="text-sm text-slate-600">
              Demo: Gunakan email dan password 'password'
            </p>
          </div>
        </form>
      </div>
